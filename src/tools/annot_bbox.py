...
def _sort_expt(pts):
  ...
  for i in range(4):
    ...
    if pts[i][1] > pts[b][1]:
      b = i
  return [pts[t], pts[l], pts[b], pts[r]]

def _expt2bbox(expt):
  expt = np.array(expt, dtype=np.int32)
  return [int(expt[:, 0].min()), int(expt[:, 1].min()), 
          int(expt[:, 0].max()), int(expt[:, 1].max())]

def save_txt(txt_name, pts_cls):
  ret = [np.array(pts_cls[i][:4], dtype=np.int32).reshape(8).tolist() \
               + [pts_cls[i][4]] for i in range(len(pts_cls))]
